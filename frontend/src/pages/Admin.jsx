...
import { isSameCity, CITIES as CITY_LIST } from '../constants/cities'
...
export default function Admin() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setStats({
        ...
        byRole: usersRes.data.reduce((acc, u) => { acc[u.role] = (acc[u.role] || 0) + 1; return acc }, {}),
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      loadData()
    } catch (err) { alert(err.response?.data?.detail || 'Lỗi tạo người dùng') }
  ...
  const handleAssign = async () => {
    setAssigning(true)
    try {
      await api.patch(`/admin/children/${assignModal.child.id}/assign`, { specialist_id: assigningId || null })
      setAssignModal(null); loadData()
    } catch (err) { alert(err.response?.data?.detail || 'Lỗi phân công') }
  ...
  const getSortedSpecialists = (childRegion) => {
    ...
    return [...specialists].sort((a, b) => {
      const aMatch = isSameCity(a.city, childRegion)
      const bMatch = isSameCity(b.city, childRegion)
  ...
  const filteredChildren = children.filter(c => {
    ...
    if (cityFilter && !isSameCity(c.region, cityFilter)) return false
    return true
  })
  ...
  const TABS = [
    ...
    { id: 'assign',   label: `🔗 Phân công${stats?.unassigned ? ` (${stats.unassigned})` : ''}` },
  ]
      ...
      <div style={S.tabs}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            ...
            cursor: 'pointer', fontSize: 14, fontWeight: tab === t.id ? 600 : 400, whiteSpace: 'nowrap',
          }}>{t.label}</button>
        ))}
      </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    {[
                      ...
                      <div key={f.key}>
                        ...
                        <input type={f.type} value={newUser[f.key]} onChange={e => setNewUser(p => ({ ...p, [f.key]: e.target.value }))} style={S.input} />
                      </div>
                    ))}
                    <div>
                  ...
                  {users.map((u, i) => {
                    ...
                    return (
                      <tr key={u.id} style={{ borderBottom: '1px solid #0f172a', background: i % 2 === 0 ? 'transparent' : '#0f172a' }}>
                        ...
                        <td style={{ padding: '12px 16px' }}>
                          <button onClick={() => handleToggle(u.id, u.username)} disabled={isAdmin || toggling === u.id}
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                <select value={assignFilter} onChange={e => setAssignFilter(e.target.value)} style={{ ...S.select, width: 'auto', fontSize: 13 }}>
                ...
                <select value={cityFilter} onChange={e => setCityFilter(e.target.value)} style={{ ...S.select, width: 'auto', fontSize: 13 }}>
            ...
            <div style={{ display: 'flex', gap: 16, marginBottom: 16, fontSize: 12, color: '#64748b', flexWrap: 'wrap' }}>
              <span>⭐ = Cùng khu vực &nbsp;|&nbsp; ⚠ = Khác khu vực</span>
            </div>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
                  <div style={{ background: '#1e293b', borderRadius: 20, padding: 32, width: 460, border: '1px solid #334155', maxHeight: '80vh', overflowY: 'auto' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 320, overflowY: 'auto', marginBottom: 16 }}>
                      <div onClick={() => setAssigningId('')}
                        style={{ padding: '10px 14px', borderRadius: 8, cursor: 'pointer', border: `2px solid ${assigningId === '' ? '#ef4444' : '#334155'}`, background: assigningId === '' ? '#450a0a' : 'transparent' }}>
                        <span style={{ color: '#ef4444', fontSize: 13 }}>— Bỏ phân công</span>
                      </div>

                      {sortedSpecs.map(s => {
                        const same = isSameCity(s.city, assignModal.child.region)
                        const isSel = assigningId === s.id
                        return (
                          <div key={s.id} onClick={() => setAssigningId(s.id)}
                            style={{ padding: '12px 14px', borderRadius: 8, cursor: 'pointer', border: `2px solid ${isSel ? '#3b82f6' : same ? '#22c55e' : '#334155'}`, background: isSel ? '#1e3a5f' : same ? '#052e16' : 'transparent', transition: 'all 0.15s' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              <div>
                                <span style={{ color: '#e2e8f0', fontWeight: 600, fontSize: 14 }}>{same ? '⭐ ' : '🩺 '}{s.full_name}</span>
                                {s.city && <span style={{ marginLeft: 8, fontSize: 12, color: same ? '#22c55e' : '#64748b' }}>📍 {s.city}</span>}
                              </div>
                              {same && <span style={{ fontSize: 11, background: '#14532d', color: '#22c55e', padding: '2px 8px', borderRadius: 10, fontWeight: 600 }}>Cùng khu vực</span>}
                            </div>
                            {s.email && <div style={{ color: '#64748b', fontSize: 12, marginTop: 4 }}>{s.email}</div>}
                          </div>
                        )
                      })}
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', minWidth: 620 }}>
                ...
                <tbody>
                  {filteredChildren.length === 0 ? (
                    <tr><td colSpan={6} style={{ padding: 40, textAlign: 'center', color: '#64748b' }}>Không có trẻ nào</td></tr>
                  ) : filteredChildren.map((c, i) => {
                    const specUser = users.find(u => u.id === c.specialist_id)
...
// CitySelect helper
function CitySelect({ value, onChange, style }) {
  return (
